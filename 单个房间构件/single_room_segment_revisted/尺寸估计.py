...
def main():
    #语义分割\实例分割
    real_data_path = r'E:\scientific_research\code\pycharm_code\PointCloud\点云逆向建模\PC_I\data\gt\Room-1-Kitchen_gt-s3dis-1.txt'
    path = r'E:\scientific_research\code\pycharm_code\PointCloud\点云逆向建模\PC_I\data\pre\Room-1-Kitchen_pointnext-xl-s3dis-1.txt'
    #创建房间信息文件夹
    room_dir = path.split('\\')[-1].split('_')[0] + '-information'
    if not os.path.isdir(room_dir):
        os.mkdir(room_dir)
    #先进行分割
    point_vis(real_data_path)
    point_vis(path)
    wall_res, door_res, window_res, floor_res, ceiling_res = segment(path)
    #利用墙体数据进行轴对齐, 输入整体房间进行R求解 对整体房间进行旋转 进行尺寸估计
    # 旋转矩阵的求取
    temp_wall_all = np.zeros((1,3))
    for i in range(len(wall_res)):
        wall_point_cloud = wall_res[i]
        temp_wall_all = np.vstack((temp_wall_all, wall_point_cloud))
    wall_all = np.delete(temp_wall_all, 0, axis=0)
    _, R, obb_center = dataClean(wall_all)
    #尺寸估计
    wall_count, door_count, window_count, floor_count = 0, 0, 0, 0
    for element_res in [wall_res, door_res, window_res, floor_res]:
        for res in element_res:
            res = rotate_point_cloud(res, R, obb_center)
            if element_res == wall_res:
                element_name = 'wall' + '_' + str(wall_count)
                wall_count = wall_count + 1
            elif element_res == door_res:
                element_name = 'door' + '_' + str(door_count)
                door_count = door_count + 1
            elif element_res == window_res:
                element_name = 'window' + '_' + str(window_count)
                window_count = window_count + 1
            elif element_res == floor_res:
                element_name = 'floor' + '_' + str(window_count)
                floor_count = floor_count + 1

            element_thickness, element_height, element_length, vertex_set = wall_se_aabb(res)
            with open(room_dir + '\\' + element_name + '.txt', 'w', encoding="utf-8") as f:
                f.write('element名称：' + '\n' + element_name + '\n')
                f.write('element高度：' + '\n' + str(element_height) + '\n')
                f.write('element长度：' + '\n' + str(element_length) + '\n')
