...
def main():
    input_file_path = r'D:\ctz\code\PC_I\PCD2BIM_paper\wall_rh\data_wall'  # 输入墙体点云文件路径
    output_file_path = r'D:\ctz\code\PC_I\PCD2BIM_paper\wall_rh\data_wall_对齐'
    ...
    for i in range(len(wall_name)):
        aligned_point_cloud = rotate_point_cloud(wall_datas[i], R, obb_center)
        np.savetxt(os.path.join(output_file_path, wall_name[i]), aligned_point_cloud)
# 示例使用
