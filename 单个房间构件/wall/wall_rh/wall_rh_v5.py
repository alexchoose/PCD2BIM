...
def plane_ransac(data):
    ...
    return np.asarray([a,b,c]), np.asarray(res), np.asarray([a,b,c,d])
def wall_thickness_height(wall):
    wall_max = np.max(wall, axis=0)
    wall_min = np.min(wall, axis=0)
    temp = abs(wall_max - wall_min)
...
def near_distance_points(points):
    ...
    for i in range(len(points)):
        ...
        # 记录索引
        nearest_indices.append([nearest_index, second_nearest_index])
    return nearest_indices
def wall_rh_id(wall_data, wall_data_normals, wall_reps, threshold = 0.5):
    wall_len = len(wall_reps)
    rh = []
    nearest_reps = near_distance_points(wall_reps)
    for wall_id1 in range(wall_len):
        for wall_id2 in range(wall_id1+1, wall_len):
            ...
            if axis1 == axis2 :#法向量最大值相同
                dist_axis = min(abs(wall_max1[axis1] - wall_max2[axis1]), abs(wall_min1[axis1] - wall_min2[axis1]))
                if dist_axis < threshold:  #曼哈顿距离
...
def main():
    start = time.time()
    input_path = r'E:\scientific_research\code\pycharm_code\PointCloud\点云逆向建模\PC_I\PCD2BIM_paper\wall\wall_rh\data_wall_对齐'
    output_path_dir = r'E:\scientific_research\code\pycharm_code\PointCloud\点云逆向建模\PC_I\PCD2BIM_paper\wall\wall_rh\data_wall_final_information_notrefinement'
    ...
    #**************************************************将结果写入其他文件**************************************************#
    wall_name_final = [x for i, x in enumerate(wall_name) if i not in wall_datas_del_id]
    wall_data_clean_final = [x for i, x in enumerate(wall_data_clean_final_temp) if i not in wall_datas_del_id]
    points_save_path = r'E:\scientific_research\code\pycharm_code\PointCloud\点云逆向建模\PC_I\PCD2BIM_paper\wall\wall_rh\data_wall_rh后'
    for i in range(len(wall_data_clean_final)):
        np.savetxt(os.path.join(points_save_path, wall_name[i]), wall_data_clean_final[i])
    wall_thicknesses, wall_heights, wall_lengths, wall_vp = [], [], [], []
    for i in range(len(wall_data_clean_final)):
        print(output_path_dir+'\\'+wall_name_final[i].replace('.txt', '_information') + '.txt')
        thick_temp, height_temp, wall_length_temp, vp_temp = aabb_(wall_data_clean_final[i])
        wall_thicknesses.append(thick_temp), wall_heights.append(height_temp), wall_lengths.append(wall_length_temp), wall_vp.append(vp_temp)
        with open(output_path_dir+'\\'+wall_name_final[i].replace('.txt', '_information') + '.txt', 'w', encoding= "utf-8") as f:
            f.write('墙体名称：' + '\n' +wall_name_final[i].replace('.txt', '_information') + '\n')
            f.write('墙体高度：'+'\n'+str(height_temp) + '\n')
            f.write('墙体长度：'+'\n'+str(wall_length_temp) + '\n')
            f.write('墙体厚度：'+'\n'+str(thick_temp) + '\n')
            f.write('墙体包围框顶点：'+ '\n')
            for i in range(len(vp_temp)):
                f.write(str(vp_temp[i]) + '\n')
