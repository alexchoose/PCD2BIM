...
def wall_thickness_height(wall):
    ...
    height_temp = temp[2]
    x_min, x_max = np.min(wall[:, 0]), np.max(wall[:, 0])
    y_min, y_max = np.min(wall[:, 1]), np.max(wall[:, 1])
    z_min, z_max = np.min(wall[:, 2]), np.max(wall[:, 2])
    range = [[x_min, x_max],[y_min, y_max], [z_min, z_max]]
    return thickness_temp, height_temp, range
def jiaodu(x, y):
    l_x = np.sqrt(x.dot(x))
    l_y = np.sqrt(y.dot(y))
    # 计算两个向量的点积
        ...
        for wall_id2 in range(wall_id1+1, wall_len):
            ...
            if jiaodu(normal1, normal2) < 35 or jiaodu(normal1, normal2) > 145 :
                if np.max(abs(v1-v2)) < 0.5:  #曼哈顿距离
                    ...
                    # 针对不同axis进行墙体的rh
                    if axis == 1:
                        rh.append([long_wall, short_wall, axis])
                    else:
                        rh.append([short_wall, long_wall, axis])
...
def dataClean(data, distThres=0.2):
    ...
    wall_data = np.asarray(downpcd.points)
    x_min, x_max = np.min(wall_data[:, 0]), np.max(wall_data[:, 0])
    y_min, y_max = np.min(wall_data[:, 1]), np.max(wall_data[:, 1])
    z_min, z_max = np.min(wall_data[:, 2]), np.max(wall_data[:, 2])
    return generate_uniform_point_cloud([x_min,x_max], [y_min, y_max], [z_min, z_max])
def wall_refinement(wall_datas, wall_data_normals,wall_datas_range):
    for i in range(len(wall_datas)):
        wall_data, wall_data_normal, wall_data_range = wall_datas[i], wall_data_normals[i], wall_datas_range[i]
...
if __name__ == '__main__':
    ...
    wall_datas_clean, wall_data_normals, thickness, height, wall_reps , wall_datas_range = [], [], [], [], [], []
    ...
    for wall_data in wall_datas:
        ...
        thickness_temp, height_temp, wall_range_temp = wall_thickness_height(temp0)        #墙高 墙厚的计算
        wall_datas_clean.append(temp0), wall_data_normals.append(normal), thickness.append(thickness_temp), height.append(height_temp)
        wall_reps.append([wall_rep[0], wall_rep[1]]), plane_list.append(plane), wall_datas_range.append(wall_range_temp)

    # wall_2d_plot(wall_reps)    #可视化2d墙体空间分布
    rh_list = wall_rh_id(wall_datas_clean, wall_data_normals,wall_reps)
    wall_data_clean_final, thickness_final, wall_datas_del_id = wall_rh_3D(wall_datas_clean, thickness, rh_list)
    end = time.time()
    ...
    for i in range(len(wall_data_clean_final)):
        np.savetxt(wall_name[i], wall_data_clean_final[i])
