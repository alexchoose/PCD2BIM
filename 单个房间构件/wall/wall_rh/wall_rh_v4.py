...
def near_distance_points(points):
    ...
    for i in range(len(points)):
        ...
        # 记录索引
        res.append([nearest_index, second_nearest_index, third_nearest_index])
    return res
def wall_rh_id(wall_data, wall_data_normals, wall_reps, threshold = 0.3):
...
def aabb_(wall_data, vis_flag = False):
    ...
    vertex_set = np.round(vertex_set, 5)
    print("aabb包围盒的顶点为：\n", vertex_set)
...
def main():
    start = time.time()
    input_path = r'D:\ctz\code\PC_I\PCD2BIM_paper\wall_rh\data_wall_rh_test'
    output_path_dir = r'data_wall_final_information_notrefinement'
    ...
    points_save_path = r'D:\ctz\code\PC_I\PCD2BIM_paper\wall_rh\data_wall_rh后'
