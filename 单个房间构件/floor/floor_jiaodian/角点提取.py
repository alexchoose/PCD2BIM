...
def main():
    #对齐后的文件夹数据
    input_data_files = r'D:\ctz\code\PC_I\PCD2BIM_paper_lantop\单个房间构件\floor\floor_data'
    output_information_files = r'D:\ctz\code\PC_I\PCD2BIM_paper_lantop\单个房间构件\floor\floor_ifc'

    output_class_information_path = os.path.join(output_information_files, 'information')
    if not os.path.isdir(output_class_information_path):
        os.mkdir(output_class_information_path)
    all_classes_data, _ = data_dir_read(input_data_files)
    classes, classes_count = 'floor', 0
    for classes_data in all_classes_data:
        ...
        vertex_set = corner_refinement(classes_data, vertex_set)
        print(vertex_set)
