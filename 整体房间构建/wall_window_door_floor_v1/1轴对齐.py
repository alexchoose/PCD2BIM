...
def main():
    # input_wall_file_path = r'D:\ctz\code\PC_I\PCD2BIM_paper_lantop\wall_window_door_floor\data\wall'  # 输入墙体点云文件路径
    input_door_file_path = r'D:\ctz\code\PC_I\PCD2BIM_paper_lantop\wall_window_door_floor\data\wall'
    output__door_file_path = r'D:\ctz\code\PC_I\PCD2BIM_paper_lantop\wall_window_door_floor\data_对齐\wall'
    # wall_datas, wall_name = data_dir_read(input_wall_file_path)
    # #输入整体房间进行R求解
    ...
    # #高度求解
    # wall_datas, wall_name = data_dir_read(input_door_file_path)
    # all_h_temp = 0
    # for wall_data in wall_datas:
    #     if np.max(wall_data[:,2]) > all_h_temp:
    #         all_h_temp = np.max(wall_data[:,2])
    obb_center = [ -5.39262028,-0.39112799,195.89472829]
    R = [[ 0.99705275,0.07671903,0.],[-0.07671903,0.99705275,0],[ 0.,0.,1.]]
    all_h_temp = 197.13020325
    door_datas, door_name = data_dir_read(input_door_file_path)
    for i in range(len(door_name)):
        door_datas[i][:, 2] = -1 * np.max(door_datas[i][:, 2]) + all_h_temp + door_datas[i][:, 2]
        aligned_point_cloud = rotate_point_cloud(door_datas[i], R, obb_center)
        np.savetxt(os.path.join(output__door_file_path, door_name[i]), aligned_point_cloud)
# 示例使用
